...
% script for empirical level ensemble regression

% Add caselist
listDir = fullfile('/local_raid1/03_user/younghyun/01_project/HCPdata');
list = load(fullfile(listDir,'/caselist_HCP.txt'));
list = censorsub(list);
list = censorsub(list); list(18) = 0; list(27) = 0; list(37) = 0; list(52) = 0; list(75) = 0; list(50) = 0; list = nonzeros(list);
...
%     atlas = ft_read_cifti(aname).parcels;
    ...
    dir_func1 = sprintf('/local_raid1/03_user/younghyun/01_project/HCPdata/Data/Functional/%d/MNINonLinear/Results/rfMRI_REST2_LR',list(sub));
    dir_func2 = sprintf('/local_raid1/03_user/younghyun/01_project/HCPdata/Data/Functional/%d/MNINonLinear/Results/rfMRI_REST1_RL',list(sub));
    
    % Load timeseries
    fname1 = fullfile(dir_func1, '/rfMRI_REST2_LR_Atlas_MSMAll_hp2000_clean.dtseries.nii');
...
%     tsmean = [tsmean_ctx; tsmean_sub];
    ...
    oAlpha = 0.5;
    command = sprintf('java  -jar causal-cmd-1.3.0-jar-with-dependencies.jar --algorithm fask --test fisher-z-test --orientationAlpha %d --faskAdjacencyMethod 1 --alpha 0.01 --data-type continuous --dataset BOLD2.txt --delimiter comma --no-header --prefix FASK2', oAlpha);
    system(command)
    FASK = Tetrad2Matrix('FASK2.txt','directed')';
    FASKs = cat(3,FASKs,FASK);
    
    alpha = 1*10^-3; 
    command = sprintf('java -jar causal-cmd-1.3.0-jar-with-dependencies.jar --algorithm fas --stableFAS --test fisher-z-test --alpha %d --data-type continuous --dataset BOLD2.txt --delimiter comma --no-header --prefix FAS2',alpha);
    system(command)
    FAS = Tetrad2Matrix('FAS2.txt','undirected');
    ...
    predicted = reshape(temp,360,360);
    
    predicteds = cat(3,predicteds,predicted);
end
toc
save('Ensemble_MMP360_retest.mat')
